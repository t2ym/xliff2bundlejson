  ...
  if (this.polymer) {
    ...
    for(i = 0; i < fields.length; i++){
      ...
      while (paths.length > 0) {
        if (paths.length === 1) {
          cursor[paths.shift()] = fields[i].target[0];
          break;
        }
        else {
...
Xliff2JSON.prototype._parseJSONAndDecorate = function(json,options,cb){
  ...
  var sources = json[''];
  if (this.polymer) {
    translations = flat.flatten(translations, { maxDepth: this.maxDepth, safe: false });
    delete sources.meta;
    sources = flat.flatten(sources, { maxDepth: this.maxDepth, safe: false });
