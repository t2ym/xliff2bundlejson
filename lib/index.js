...
function Xliff2JSON(options){
    ...
    this.xliffTemplate = options.xliffTemplate ||
      (options.polymer ? __dirname+'/xliff_template_polymer.xml' : __dirname+'/xliff_template.xml');
  ...
  for(var i=0;i<translationKeys.length;i++){
    ...
    var unit = {
        "$": (this.polymer ?
              ...
              {"id": (i+1).toString() }),
        "source": (this.polymer ? (typeof sources[translationKeys[i]] !== 'undefined'
